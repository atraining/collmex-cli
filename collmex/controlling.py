...
Berechnet KPIs, Liquiditätsvorschau und Soll-Ist-Vergleiche
...
class ControllingEngine:
    """Controlling-Engine für KPIs, Liquiditätsplanung und Soll-Ist-Vergleich.

    Arbeitet ausschliesslich mit Daten aus der Collmex-API.
    Alle Geldbeträge als Decimal für kaufmännische Exaktheit.
    """
    ...
    def _get_balance(self, account: int, year: int, period: int) -> Decimal:
        ...
        Gibt Decimal(0) zurück wenn das Konto keine Daten hat.
        """
    ...
    def dashboard(self) -> dict:
        ...
        Returns:
            dict mit Schlüssel:
                ...
                - umsatz_monat: Erlöse 8100-8700 aktueller Monat
                - kosten_monat: Aufwendungen 4000-4999 aktueller Monat
                - ergebnis_monat: umsatz - kosten
                - liquidität_1: (Bank + Kasse) / kurzfr. Verbindlichkeiten
        ...
        # Erlöse: 8100-8700 (Ertragskonten: Haben-Saldo ist positiv)
        ...
        # Liquidität 1. Grades: (Bank + Kasse) / kurzfr. Verbindl.
        if offene_verbindlichkeiten > Decimal("0"):
            liquidität_1 = ((kontostand + kasse) / offene_verbindlichkeiten).quantize(
                Decimal("0.01")
            )
        else:
            liquidität_1 = Decimal("0")

        # DSO: Days Sales Outstanding = (Forderungen / Umsatz) * 365
        if umsatz_monat > Decimal("0"):
            # Jahresumsatz schätzen: Monatsumsatz * 12
    ...
    def liquiditätsvorschau(self, wochen: int = 13) -> list[dict]:
        """Erstellt eine wöchentliche Liquiditätsvorschau.

        Basiert auf offenen Posten (OPEN_ITEMS_GET) und verteilt
        erwartete Ein-/Ausgänge nach Fälligkeitsdatum auf Kalenderwochen.
        ...
        """
        heute = date.today()
        # Wochenanfänge berechnen (Montag)
        ...
        for row in rows:
            ...
            try:
                ...
                fällig_str = row[_IDX_FAELLIG] if row[_IDX_FAELLIG] else ""
            except Exception:
                continue

            if not fällig_str or len(fällig_str) != 8:
                continue
            ...
            if fällig < montag or fällig >= ende:
                continue

            # Woche bestimmen
            delta_tage = (fällig - montag).days
            ...
            if 0 <= wochen_index < len(wochen_liste):
                ...
                # Debitoren (Personenkonten 10000-69999): Eingänge
                # Kreditoren (Personenkonten 70000+): Ausgänge
