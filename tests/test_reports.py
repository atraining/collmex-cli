"""Tests fuer collmex Reports (BWA, SuSa, OP, Saeumige Kunden).
...
def _make_accbal_row(
    ...
    soll_umsatz/haben_umsatz/anfangsbestand werden ignoriert (fuer
    Rueckwaertskompatibilitaet der Aufrufe behalten).
    """
    return [
        "ACC_BAL",  # 0: Satzart
...
def test_bwa_berechnung_erloese_minus_kosten(engine, mock_client):
    """BWA: Erloese minus Kosten = Betriebsergebnis."""
    mock_client.get_balances.return_value = [
        _make_accbal_row(8400, "Erloese 19%", saldo="-10000,00"),
        _make_accbal_row(4100, "Loehne", saldo="3000,00"),
        _make_accbal_row(4210, "Miete", saldo="1000,00"),
        _make_accbal_row(4400, "Buerobedarf", saldo="500,00"),
    ]

    result = engine.bwa(2026, 3)

    assert result["umsatzerloese"] == Decimal("10000.00")
...
def test_bwa_ohne_erloese(engine, mock_client):
    """BWA: Nur Kosten, keine Erloese ergibt negatives Ergebnis."""
    mock_client.get_balances.return_value = [
        _make_accbal_row(4100, "Loehne", saldo="2000,00"),
        _make_accbal_row(4210, "Miete", saldo="800,00"),
    ]

    result = engine.bwa(2026, 3)

    assert result["umsatzerloese"] == Decimal("0")
...
def test_bwa_positionen_reihenfolge(engine, mock_client):
    ...
    mock_client.get_balances.return_value = [
        _make_accbal_row(8400, "Erloese 19%", saldo="-5000,00"),
        _make_accbal_row(4100, "Loehne", saldo="1000,00"),
    ]

    result = engine.bwa(2026, 3)
    positionen = result["positionen"]

    assert positionen[0]["bezeichnung"] == "Umsatzerloese"
...
def test_bwa_summe_kosten(engine, mock_client):
    ...
    mock_client.get_balances.return_value = [
        ...
        _make_accbal_row(4400, "Buerobedarf", saldo="200,00"),
...
def test_bwa_alle_bereiche_vorhanden(engine, mock_client):
    """BWA-Ergebnis enthaelt alle definierten Bereiche."""
    mock_client.get_balances.return_value = []
    result = engine.bwa(2026, 3)

    for bereich in BWA_BEREICHE:
        if bereich == "umsatzerloese":
...
def test_susa_sortierung(engine, mock_client):
    ...
    mock_client.get_balances.return_value = [
        _make_accbal_row(4400, "Buerobedarf", saldo="100,00"),
        _make_accbal_row(1200, "Bank", saldo="5000,00"),
        _make_accbal_row(8400, "Erloese", saldo="-8000,00"),
    ]
...
def test_op_altersstruktur(engine, mock_client):
    ...
    assert alters["ueberfaellig_30"] == Decimal("200.00")
    assert alters["ueberfaellig_60"] == Decimal("300.00")
    assert alters["ueberfaellig_90"] == Decimal("400.00")
...
def test_saeumige_korrekte_sortierung(engine, mock_client):
    """Saeumige Kunden: Sortierung nach Tagen ueberfaellig (absteigend)."""
    heute = date.today()
    tag_10 = (heute - timedelta(days=10)).strftime("%Y%m%d")
...
def test_saeumige_mahnstufen(engine, mock_client):
    """Saeumige Kunden: Mahnstufen sind korrekt."""
    heute = date.today()
    tag_10 = (heute - timedelta(days=10)).strftime("%Y%m%d")
    tag_45 = (heute - timedelta(days=45)).strftime("%Y%m%d")
    tag_75 = (heute - timedelta(days=75)).strftime("%Y%m%d")
...
def test_saeumige_keine_ueberfaelligen(engine, mock_client):
    """Saeumige Kunden: Leere Liste wenn niemand ueberfaellig."""
...
def test_saeumige_nur_debitoren(engine, mock_client):
    """Saeumige Kunden: Kreditoren werden nicht als saemige Kunden gelistet."""
    gestern = (date.today() - timedelta(days=1)).strftime("%Y%m%d")
...
def test_bwa_bezeichnungen_vollstaendig():
